...
void StderrLogger::do_log(Level level, std::string const& message)
{
    std::cerr << get_level_prefix(level) << message << '\n'; // Throws
    std::cerr.flush();                                       // Throws
}

void StreamLogger::do_log(Level level, std::string const& message)
{
    m_out << get_level_prefix(level) << message << '\n'; // Throws
    m_out.flush();                                       // Throws
}

void ThreadSafeLogger::do_log(Level level, std::string const& message)
{
    LockGuard l(m_mutex);
    Logger::do_log(m_base_logger, level, message); // Throws
}

void PrefixLogger::do_log(Level level, std::string const& message)
{
